...
const instantMode = false;
  ...
  function intializeCanvas({
    ...
    const clearDom = clear ? `<button id="${id}-clear" class="iconButton" title="Clear canvas">${clearSvg}</button>` : "";
    const resetDom = reset ? `<button id="${id}-reset" class="iconButton" title="Reset to default scene">${resetSvg}</button>` : "";
    const sunMoonDom = toggleSun ? `<button id="${id}-sun" class="iconButton" title="Toggle sun">${sunMoonSvg}</button>` : "";
    ...
    <div style="display: flex; gap: 20px;">
      ...
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
          <div id="${id}-color-picker" style="display: flex; flex-direction: column;  border: none; margin: 1px;">
  ...
  class PaintableCanvas {
    ...
    getMousePos(e) {
      ...
      if (e.touches) {
        return {
          x: (e.touches[0].clientX - rect.left) * dx,
          y: (e.touches[0].clientY - rect.top) * dy
        };
      }
